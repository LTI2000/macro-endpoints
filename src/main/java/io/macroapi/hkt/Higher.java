 ...
 *       {@code class Foo<A> implements Higher<Foo.Witness, A>};</li>
 *   <li>the nested {@code Witness} type is uninhabited and exists only as a type-level tag;</li>
 *   <li>a static {@code narrow(Higher<Foo.Witness, A>)} method performs the (always safe, but
 ...
 * so the only value that can ever have static type {@code Higher<Foo.Witness, A>} is a {@code Foo<A>}.</p>
 *
 * @param <F> the witness type standing for the type constructor
 * @param <A> the type the constructor is applied to
 */
public interface Higher<F, A> {
}
